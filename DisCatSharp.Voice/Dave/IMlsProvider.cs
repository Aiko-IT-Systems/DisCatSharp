...
internal interface IMlsProvider
{
	...
	/// <summary>Initialises a new MLS group for this session.</summary>
	...
	/// <summary>Stores the external sender credential received from the server (binary OP 25).</summary>
	void SetExternalSender(byte[] externalSenderBytes);

	/// <summary>Returns the serialised MLS key package to send to the server during group join.</summary>
	byte[] GetKeyPackage();
	...
	/// <summary>Applies an MLS commit received from the server (binary OP 29).</summary>
	...
	/// <summary>Processes an MLS welcome message, joining the group as a new member (binary OP 30).</summary>
	...
	/// <summary>Returns a <see cref="DaveRatchetInstaller"/> for the local participant's own outbound ratchet for this epoch.</summary>
	DaveRatchetInstaller GetOwnRatchetInstaller();

	/// <summary>Resets all MLS state. Called on reconnect, session downgrade, or invalid commit.</summary>
	void Reset();
}
