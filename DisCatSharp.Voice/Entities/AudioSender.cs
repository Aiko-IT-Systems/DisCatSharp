...
internal class AudioSender : IDisposable
{
	...
	public ulong GetTrueSequenceAfterWrapping(ushort originalSequence)
	{
		...
		switch (this._currentSequenceWrapState)
		{
			...
			case SequenceWrapState.Normal:
				// sequence is in the ordinary range — nothing to do, no state change.
				break;
			...
			case SequenceWrapState.AssumeNextLowSequenceIsOverflow:
				// we expected overflow but the sequence number came back to normal range instead
				// (e.g. an out-of-order packet resolved itself).  return to normal tracking.
			...
			case SequenceWrapState.AssumeNextHighSequenceIsOutOfOrder:
				// sequence is still in the low zone — could be the very start of a stream or a
				// packet that arrived just after an overflow.  no state change; pass through.
				break;

			default:
				throw new ArgumentOutOfRangeException(null, $"CurrentSequenceWrapState was out of range: {this._currentSequenceWrapState}");
		}
