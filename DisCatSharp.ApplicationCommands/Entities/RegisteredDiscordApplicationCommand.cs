	...
	internal RegisteredDiscordApplicationCommand(DiscordApplicationCommand parent)
	{
		...
		try
		{
			if (ApplicationCommandsExtension.CommandMethods.Any(x => x.CommandId == this.Id))
			{
				...
				this.CustomAttributes = [.. this.CommandMethod.GetCustomAttributes().Where(x => !x.GetType().Namespace.StartsWith("DisCatSharp", StringComparison.Ordinal))];
			}
			...
			else if (ApplicationCommandsExtension.GroupCommands.Any(x => x.CommandId == this.Id))
			{
				...
				this.CustomAttributes = [.. this.CommandType.GetCustomAttributes().Where(x => !x.GetType().Namespace.StartsWith("DisCatSharp", StringComparison.Ordinal))];
			}
		}
		catch (Exception)
		{
