...
function Build-All([string] $target_dir_path, [string] $version_suffix, [string] $build_number, [string] $bcfg)
{
    ...
    & dotnet clean -v minimal -c "$bcfg" DisCatSharp.Tools/DisCatSharp.Analyzer/DisCatSharp.Analyzer.Package/DisCatSharp.Analyzer.Package.csproj | Out-Host
    ...
    & dotnet restore --no-cache -f -v minimal DisCatSharp.Tools/DisCatSharp.Analyzer/DisCatSharp.Analyzer.Package/DisCatSharp.Analyzer.Package.csproj | Out-Host
    if ($LastExitCode -ne 0)
    {
