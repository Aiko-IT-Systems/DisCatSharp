...
public class BasicEventHandlerTests
{
	...
	private bool IsEventRegistered(string name)
	{
		...
		var asyncEvent = typeof(DiscordClient).GetField(name, BindingFlags.NonPublic | BindingFlags.Instance)
			?.GetValue(this._client);
		dynamic handlers = asyncEvent?.GetType().GetField("_handlers", BindingFlags.NonPublic | BindingFlags.Instance)
			                   ?.GetValue(asyncEvent)
