...
public sealed partial class DiscordClient
{
	...
	internal async Task HandleDispatchAsync(GatewayPayload payload)
	{
		...
		switch (payload.EventName.ToLowerInvariant())
		{
			...
			case "message_update":
				...
				await this.OnMessageUpdateEventAsync(DiscordJson.DeserializeObject<DiscordMessage>(payloadString, this), dat, dat["author"] != null ? DiscordJson.DeserializeObject<TransportUser>(dat["author"]!.ToString(), this) : null!, mbr!, refUsr!, refMbr!).ConfigureAwait(false);
				break;
			...
			case "webhooks_update":
				gid = (ulong)dat["guild_id"];
				cid = (ulong)dat["channel_id"];
				await this.OnWebhooksUpdateAsync(cid, gid).ConfigureAwait(false);
				break;
	...
	internal async Task OnReadyEventAsync(ReadyPayload ready, JArray rawGuilds)
	{
		...
		else
		{
			...
			await this.RaiseEventAsync(this.GuildDownloadCompletedEv, new(this.Guilds, true, this.ServiceProvider)).ConfigureAwait(false);
	...
	internal Task OnResumedAsync()
	{
		...
		return this.RaiseEventAsync(this._resumed, new(this.ServiceProvider));
	}
	...
	internal async Task OnChannelDeleteEventAsync(DiscordChannel channel)
	{
		...
		if (channel.Type == ChannelType.Group || channel.Type == ChannelType.Private)
		{
			...
			await this.RaiseEventAsync(this._dmChannelDeleted, new(this.ServiceProvider)
			{
				Channel = dmChannel
			}).ConfigureAwait(false);
		}
	...
	internal async Task OnChannelPinsUpdateAsync(ulong? guildId, ulong channelId, DateTimeOffset? lastPinTimestamp)
	{
		...
		await this.RaiseEventAsync(this._channelPinsUpdated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnVoiceChannelStatusUpdateAsync(ulong guildId, ulong channelId, string? status)
	{
		...
		await this.RaiseEventAsync(this._voiceChannelStatusUpdated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnVoiceChannelStartTimeUpdateAsync(ulong guildId, ulong channelId, DateTimeOffset? voiceStartTime)
	{
		...
		await this.RaiseEventAsync(this._voiceChannelStartTimeUpdated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildCreateEventAsync(DiscordGuild guild, JArray rawMembers, IEnumerable<DiscordPresence>? presences, bool hasSoundboardSounds = false)
	{
		...
		if (presences != null)
			foreach (var xp in presences)
				this.CachePresence(guild, this.PreparePresence(xp, guild));
		...
		if (exists)
			await this.RaiseEventAsync(this._guildAvailable, new(this.ServiceProvider)
			{
				Guild = guild
			}).ConfigureAwait(false);
		else
			await this.RaiseEventAsync(this._guildCreated, new(this.ServiceProvider)
			{
				Guild = guild
			}).ConfigureAwait(false);

		if (dcompl && !old)
			await this.RaiseEventAsync(this.GuildDownloadCompletedEv, new(this.Guilds, false, this.ServiceProvider)).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildUpdateEventAsync(DiscordGuild guild, JArray rawMembers, bool hasSoundboardSounds = false)
	{
		...
		await this.RaiseEventAsync(this._guildUpdated, new(this.ServiceProvider)
		{
	...
	internal async Task OnGuildDeleteEventAsync(DiscordGuild guild)
	{
		if (guild.IsUnavailable)
		{
			...
			await this.RaiseEventAsync(this._guildUnavailable, new(this.ServiceProvider)
			{
				Guild = guild,
				Unavailable = true
			}).ConfigureAwait(false);
		...
		else
		{
			...
			await this.RaiseEventAsync(this._guildDeleted, new(this.ServiceProvider)
			{
				Guild = gld
			}).ConfigureAwait(false);

			this.RemovePresences(gld);
		}
	}
	...
	internal async Task OnGuildAuditLogEntryCreateEventAsync(DiscordGuild guild, JObject auditLogCreateEntry)
	{
		try
		{
			...
			if (parsedEntry is not null)
				await this.RaiseEventAsync(this._guildAuditLogEntryCreated, new(this.ServiceProvider)
				{
					Guild = guild,
	...
	internal async Task OnGuildEmojisUpdateEventAsync(DiscordGuild guild, IEnumerable<DiscordEmoji> newEmojis)
	{
		...
		await this.RaiseEventAsync(this._guildEmojisUpdated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnStickersUpdatedAsync(IEnumerable<DiscordSticker> newStickers, ulong guildId)
	{
		...
		await this.RaiseEventAsync(this._guildStickersUpdated, sea).ConfigureAwait(false);
	}
	...
	internal async Task OnAutomodRuleCreated(AutomodRule newRule)
	{
		...
		await this.RaiseEventAsync(this._automodRuleCreated, sea).ConfigureAwait(false);
	}
	...
	internal async Task OnAutomodRuleUpdated(AutomodRule updatedRule)
	{
		...
		await this.RaiseEventAsync(this._automodRuleUpdated, sea).ConfigureAwait(false);
	}
	...
	internal async Task OnAutomodRuleDeleted(AutomodRule deletedRule)
	{
		...
		await this.RaiseEventAsync(this._automodRuleDeleted, sea).ConfigureAwait(false);
	}
	...
	internal async Task OnAutomodActionExecuted(DiscordGuild guild, JObject rawPayload)
	{
		...
		await this.RaiseEventAsync(this._automodActionExecuted, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildBanAddEventAsync(TransportUser user, DiscordGuild guild)
	{
		...
		await this.RaiseEventAsync(this._guildBanAdded, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildBanRemoveEventAsync(TransportUser user, DiscordGuild guild)
	{
		...
		await this.RaiseEventAsync(this._guildBanRemoved, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildScheduledEventCreateEventAsync(DiscordScheduledEvent scheduledEvent, DiscordGuild guild)
	{
		...
		await this.RaiseEventAsync(this._guildScheduledEventCreated, new(this.ServiceProvider)
		{
	...
	internal async Task OnGuildScheduledEventUpdateEventAsync(DiscordScheduledEvent scheduledEvent, DiscordGuild guild)
	{
		...
		if (scheduledEvent.Status == ScheduledEventStatus.Completed)
		{
			...
			await this.RaiseEventAsync(this._guildScheduledEventDeleted, new(this.ServiceProvider)
			{
				ScheduledEvent = scheduledEvent,
				Guild = guild,
		...
		else
		{
			...
			await this.RaiseEventAsync(this._guildScheduledEventUpdated, new(this.ServiceProvider)
			{
	...
	internal async Task OnGuildScheduledEventUserAddedEventAsync(ulong guildScheduledEventId, ulong userId, ulong? exceptionId, DiscordGuild guild)
	{
		...
		await this.RaiseEventAsync(this._guildScheduledEventUserAdded, new(this.ServiceProvider)
		{
			ScheduledEvent = scheduledEvent,
			Guild = guild,
			User = user,
	...
	internal async Task OnGuildScheduledEventUserRemovedEventAsync(ulong guildScheduledEventId, ulong userId, ulong? exceptionId, DiscordGuild guild)
	{
		...
		await this.RaiseEventAsync(this._guildScheduledEventUserRemoved, new(this.ServiceProvider)
		{
			ScheduledEvent = scheduledEvent,
			Guild = guild,
			User = user,
	...
	internal async Task OnGuildScheduledEventExceptionCreateEventAsync(DiscordScheduledEventException exception, DiscordGuild guild)
	{
		...
		await this.RaiseEventAsync(this._guildScheduledEventExceptionCreated, new(this.ServiceProvider)
		{
			Exception = exception,
			Guild = guild
		}).ConfigureAwait(false);
	...
	internal async Task OnGuildScheduledEventExceptionUpdateEventAsync(DiscordScheduledEventException exception, DiscordGuild guild)
	{
		...
		await this.RaiseEventAsync(this._guildScheduledEventExceptionUpdated, new(this.ServiceProvider)
		{
	...
	internal async Task OnGuildScheduledEventExceptionDeleteEventAsync(DiscordScheduledEventException exception, DiscordGuild guild)
	{
		...
		await this.RaiseEventAsync(this._guildScheduledEventExceptionDeleted, new(this.ServiceProvider)
		{
			Exception = exception,
			Guild = guild
		}).ConfigureAwait(false);
	...
	internal async Task OnGuildIntegrationCreateEventAsync(DiscordGuild guild, DiscordIntegration integration)
	{
		...
		await this.RaiseEventAsync(this._guildIntegrationCreated, new(this.ServiceProvider)
		{
	...
	internal async Task OnGuildIntegrationUpdateEventAsync(DiscordGuild guild, DiscordIntegration integration)
	{
		...
		await this.RaiseEventAsync(this._guildIntegrationUpdated, new(this.ServiceProvider)
		{
			Integration = integration,
			Guild = guild
		}).ConfigureAwait(false);
	...
	internal async Task OnGuildIntegrationsUpdateEventAsync(DiscordGuild guild)
	{
		...
		await this.RaiseEventAsync(this._guildIntegrationsUpdated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildAppliedBoostsUpdateEventAsync(ulong boostId, DiscordGuild guild, ulong userId, DateTimeOffset? pauseEndsAt, DateTimeOffset? endsAt, bool ended)
	{
		...
		await this.RaiseEventAsync(this._guildAppliedBoostsUpdated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildAppliedBoostsCreateEventAsync(ulong boostId, ulong guildId, ulong userId, DateTimeOffset? pauseEndsAt, DateTimeOffset? endsAt, bool ended)
	{
		...
		await this.RaiseEventAsync(this._guildAppliedBoostsCreated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildAppliedBoostsDeleteEventAsync(ulong boostId, ulong guildId, ulong userId, DateTimeOffset? pauseEndsAt, DateTimeOffset? endsAt, bool ended)
	{
		...
		await this.RaiseEventAsync(this._guildAppliedBoostsDeleted, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildPowerupEntitlementsCreateEventAsync(List<DiscordEntitlement> entitlements, ulong guildId)
	{
		...
		await this.RaiseEventAsync(this._guildPowerupEntitlementsCreated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildPowerupEntitlementsDeleteEventAsync(List<DiscordEntitlement> entitlements, ulong guildId)
	{
		...
		await this.RaiseEventAsync(this._guildPowerupEntitlementsDeleted, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildIntegrationDeleteEventAsync(DiscordGuild guild, ulong integrationId, ulong? applicationId)
		=> await this.RaiseEventAsync(this._guildIntegrationDeleted, new(this.ServiceProvider)
		{
			Guild = guild,
	...
	internal async Task OnGuildMemberAddEventAsync(TransportMember member, DiscordGuild guild)
	{
		...
		await this.RaiseEventAsync(this._guildMemberAdded, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildMemberRemoveEventAsync(TransportUser user, DiscordGuild guild)
	{
		...
		await this.RaiseEventAsync(this._guildMemberRemoved, ea).ConfigureAwait(false);

		this.RemovePresence(guild.Id, user.Id);
	}
	...
	internal async Task OnGuildMemberUpdateEventAsync(TransportMember member, DiscordGuild guild, IEnumerable<ulong> roles, string nick, bool? pending)
	{
		...
		var gCollectiblesOld = old.GuildCollectibles;
		...
		var eargs = new GuildMemberUpdateEventArgs(this.ServiceProvider)
		{
			...
			GuildCollectiblesAfter = mbr.GuildCollectibles,
			...
			GuildCollectiblesBefore = gCollectiblesOld
		};

		await this.RaiseEventAsync(this._guildMemberUpdated, eargs).ConfigureAwait(false);
		...
		if (timeoutBefore is null && timeoutAfter is not null)
			await this.RaiseEventAsync(this._guildMemberTimeoutAdded, new(this.ServiceProvider)
			{
				Guild = guild,
				Target = mbr,
		...
		if (timeoutBefore is not null && timeoutAfter is not null)
			await this.RaiseEventAsync(this._guildMemberTimeoutChanged, new(this.ServiceProvider)
			{
				Guild = guild,
				Target = mbr,
		...
		if (timeoutBefore is not null && timeoutAfter is null)
			await this.RaiseEventAsync(this._guildMemberTimeoutRemoved, new(this.ServiceProvider)
			{
				Guild = guild,
				Target = mbr,
				TimeoutBefore = timeoutBefore.Value
	...
	internal async Task OnGuildMembersChunkEventAsync(JObject dat)
	{
		...
		if (dat["presences"] is not null)
		{
			...
			if (presences is not null)
				foreach (var presence in presences)
					pres.Add(this.CachePresence(guild, this.PreparePresence(presence, guild)));
		...
		await this.RaiseEventAsync(this._guildMembersChunked, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildRoleCreateEventAsync(DiscordRole role, DiscordGuild guild)
	{
		...
		await this.RaiseEventAsync(this._guildRoleCreated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildRoleDeleteEventAsync(ulong roleId, DiscordGuild guild)
	{
		...
		await this.RaiseEventAsync(this._guildRoleDeleted, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildSoundboardSoundCreateEventAsync(DiscordSoundboardSound sound, ulong guildId)
	{
		...
		await this.RaiseEventAsync(this._guildSoundboardSoundCreated, args).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildSoundboardSoundUpdateEventAsync(DiscordSoundboardSound sound, ulong guildId)
	{
		...
		await this.RaiseEventAsync(this._guildSoundboardSoundUpdated, args).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildSoundboardSoundDeleteEventAsync(ulong soundId, ulong guildId)
	{
		...
		await this.RaiseEventAsync(this._guildSoundboardSoundDeleted, args).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildSoundboardSoundsUpdateEventAsync(List<DiscordSoundboardSound> sounds, ulong guildId)
	{
		...
		await this.RaiseEventAsync(this._guildSoundboardSoundsUpdated, args).ConfigureAwait(false);
	}
	...
	internal async Task OnSoundboardSoundsEventAsync(List<DiscordSoundboardSound> sounds, ulong guildId)
	{
		...
		if (this._soundboardSounds is not null)
			await this.RaiseEventAsync(this._soundboardSounds, new(this.ServiceProvider)
			{
				GuildId = guildId,
				Guild = guild,
				Sounds = sounds
	...
	internal async Task OnInviteCreateEventAsync(ulong channelId, ulong guildId, DiscordInvite invite)
	{
		...
		await this.RaiseEventAsync(this._inviteCreated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnInviteDeleteEventAsync(ulong channelId, ulong guildId, JToken dat)
	{
		...
		await this.RaiseEventAsync(this._inviteDeleted, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildJoinRequestCreateAsync(DiscordGuild guild, JoinRequestStatusType statusType, DiscordGuildJoinRequest request)
	{
		...
		await this.RaiseEventAsync(this._guildJoinRequestCreated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildJoinRequestUpdateAsync(DiscordGuild guild, JoinRequestStatusType statusType, DiscordGuildJoinRequest request)
	{
		...
		await this.RaiseEventAsync(this._guildJoinRequestUpdated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildJoinRequestDeleteAsync(ulong requestId, ulong userId, DiscordGuild guild)
	{
		...
		await this.RaiseEventAsync(this._guildJoinRequestDeleted, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnMessageAckEventAsync(DiscordChannel chn, ulong messageId)
	{
		...
		await this.RaiseEventAsync(this._messageAcknowledged, new(this.ServiceProvider)
		{
			Message = msg
		}).ConfigureAwait(false);
	}
		...
		internal async Task OnMessageUpdateEventAsync(DiscordMessage message, JObject rawData, TransportUser author, TransportMember member, TransportUser referenceAuthor, TransportMember referenceMember)
	{
		...
		else
		{
			...
			guild = message.Channel?.Guild;
			if (rawData.TryGetValue("edited_timestamp", out _))
				message.EditedTimestampRaw = eventMessage.EditedTimestampRaw;
			if (rawData.TryGetValue("content", out _) && eventMessage.Content != null)
				message.Content = eventMessage.Content;
			if (rawData.TryGetValue("attachments", out _))
			{
				message.AttachmentsInternal.Clear();
				message.AttachmentsInternal.AddRange(eventMessage.AttachmentsInternal);
			}
			if (rawData.TryGetValue("embeds", out _))
			{
				message.EmbedsInternal.Clear();
				message.EmbedsInternal.AddRange(eventMessage.EmbedsInternal);
			}
			if (rawData.TryGetValue("components", out _))
				message.Components = eventMessage.Components;
			if (rawData.TryGetValue("mention_everyone", out _))
				message.MentionEveryone = eventMessage.MentionEveryone;
			if (rawData.TryGetValue("mentions", out _))
			{
				message.MentionedUsersInternal.Clear();
				message.MentionedUsersInternal.AddRange(eventMessage.MentionedUsersInternal);
			}
			if (rawData.TryGetValue("mention_roles", out _))
			{
				message.MentionedRoleIds = [.. eventMessage.MentionedRoleIds];
				message.MentionedRolesInternal.Clear();
				message.MentionedRolesInternal.AddRange(eventMessage.MentionedRolesInternal);
			}
			if (rawData.TryGetValue("mention_channels", out _))
			{
				message.MentionedChannelsPartial = [.. eventMessage.MentionedChannelsPartial];
				message.MentionedChannelsInternal.Clear();
				message.MentionedChannelsInternal.AddRange(eventMessage.MentionedChannelsInternal);
			}
			if (rawData.TryGetValue("flags", out _))
				message.Flags = eventMessage.Flags;
			if (rawData.TryGetValue("pinned", out _))
				message.Pinned = eventMessage.Pinned;
			if (rawData.TryGetValue("tts", out _))
				message.IsTts = eventMessage.IsTts;
			if (rawData.TryGetValue("poll", out _))
			{
				message.Poll = eventMessage.Poll;
				if (message.Poll is not null)
				{
					message.Poll.ChannelId = message.ChannelId;
					message.Poll.MessageId = message.Id;
					message.Poll.Discord = this;
				}
			}
		}
		...
		await this.RaiseEventAsync(this._messageUpdated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnMessageDeleteEventAsync(ulong messageId, ulong channelId, ulong? guildId)
	{
		...
		if (channel == null
			|| this.Configuration.Cache.MessageCacheSize == 0
			|| this.MessageCache == null
		...
		await this.RaiseEventAsync(this._messageDeleted, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnMessageBulkDeleteEventAsync(ulong[] messageIds, ulong channelId, ulong? guildId)
	{
		...
		await this.RaiseEventAsync(this._messagesBulkDeleted, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnMessagePollVoteAddEventAsync(JObject dat)
	{
		...
		var guildId = (ulong?)dat["guild_id"];
		...
		await this.RaiseEventAsync(this._messagePollVoteAdded, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnMessagePollVoteRemoveEventAsync(JObject dat)
	{
		...
		await this.RaiseEventAsync(this._messagePollVoteRemoved, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnMessageReactionRemoveEmojiAsync(ulong messageId, ulong channelId, ulong guildId, DiscordEmoji partialEmoji)
	{
		...
		if (guild == null || !guild.EmojisInternal.TryGetValue(partialEmoji.Id, out var emoji))
		{
			emoji = partialEmoji;
			emoji.Discord = this;
		}
		...
		await this.RaiseEventAsync(this._messageReactionRemovedEmoji, ea).ConfigureAwait(false);
	}
	...
	{
		thread.Discord = this;
		var guild = this.InternalGetCachedGuild(thread.GuildId);
		var parent = thread.ParentId.HasValue
			? this.InternalGetCachedChannel(thread.ParentId.Value)
			: null;
		guild?.ThreadsInternal.AddOrUpdate(thread.Id, thread, (oldThread, newThread) => newThread);

		await this.RaiseEventAsync(this._threadCreated, new(this.ServiceProvider)
		{
			Thread = thread,
			Guild = guild ?? new DiscordGuild
			{
				Id = thread.GuildId.GetValueOrDefault(),
				Discord = this
			},
			Parent = parent ?? new DiscordChannel
			{
				Id = thread.ParentId.GetValueOrDefault(),
	...
	internal async Task OnThreadUpdateEventAsync(DiscordThreadChannel thread)
	{
		...
		if (threadNew != null)
		{
			...
			if (this.Guilds != null)
			{
				var parentChannel = thread.ParentId.HasValue ? this.InternalGetCachedChannel(thread.ParentId.Value) : null;
				if (parentChannel?.Type == ChannelType.Forum)
				{
		...
		await this.RaiseEventAsync(this._threadUpdated, updateEvent).ConfigureAwait(false);
	}
	...
	internal async Task OnThreadDeleteEventAsync(DiscordThreadChannel thread)
	{
		...
		await this.RaiseEventAsync(this._threadDeleted, new(this.ServiceProvider)
		{
			Thread = thread,
			Guild = thread.Guild,
			Parent = thread.Parent,
	...
	internal async Task OnThreadListSyncEventAsync(DiscordGuild guild, IReadOnlyList<ulong?> channelIds, IReadOnlyList<DiscordThreadChannel> threads, IReadOnlyList<DiscordThreadChannelMember> members)
	{
		...
		var channels = channelIds
			.Where(x => x.HasValue)
			.Select(x => guild.GetChannel(x!.Value))
		...
		var syncedThreads = new List<DiscordThreadChannel>(threads.Count);
		foreach (var thread in threads)
		{
			thread.Discord = this;
			if (thread.GuildId is 0)
				thread.GuildId = guild.Id;

			syncedThreads.Add(guild.ThreadsInternal.AddOrUpdate(thread.Id, thread, (_, cachedThread) =>
			{
				cachedThread.Discord = this;
				cachedThread.Type = thread.Type;
				cachedThread.ThreadMetadata = thread.ThreadMetadata;
				cachedThread.ParentId = thread.ParentId;
				cachedThread.OwnerId = thread.OwnerId;
				cachedThread.Name = thread.Name;
				cachedThread.LastMessageId = thread.LastMessageId ?? cachedThread.LastMessageId;
				cachedThread.MessageCount = thread.MessageCount;
				cachedThread.MemberCount = thread.MemberCount;
				cachedThread.GuildId = thread.GuildId;
				cachedThread.LastPinTimestampRaw = thread.LastPinTimestampRaw ?? cachedThread.LastPinTimestampRaw;
				cachedThread.PerUserRateLimit = thread.PerUserRateLimit;
				cachedThread.TotalMessagesSent = thread.TotalMessagesSent;

				if (thread.AppliedTagIdsInternal is not null)
					cachedThread.AppliedTagIdsInternal = thread.AppliedTagIdsInternal;

				if (thread.ThreadMembersInternal is not null)
					cachedThread.ThreadMembersInternal = thread.ThreadMembersInternal;

				if (thread.CurrentMember is not null)
					cachedThread.CurrentMember = thread.CurrentMember;

				return cachedThread;
			}));
		}

		var syncedMembers = new List<DiscordThreadChannelMember>(members.Count);
		foreach (var member in members)
		{
			member.Discord = this;
			if (member.GuildId is 0)
				member.GuildId = guild.Id;

			syncedMembers.Add(member);

			if (!guild.ThreadsInternal.TryGetValue(member.Id, out var thread))
				continue;

			thread.CurrentMember = member;
		}

		await this.RaiseEventAsync(this._threadListSynced, new(this.ServiceProvider)
		{
			Guild = guild,
			Channels = channels.AsReadOnly(),
			Threads = syncedThreads.AsReadOnly(),
			Members = syncedMembers.AsReadOnly()
		}).ConfigureAwait(false);
	}
	...
	internal async Task OnThreadMemberUpdateEventAsync(DiscordThreadChannelMember member, CancellationToken cancellationToken = default)
	{
		...
		await this.RaiseEventAsync(this._threadMemberUpdated, new(this.ServiceProvider)
		{
			ThreadMember = member,
			Thread = thread
		}).ConfigureAwait(false);
	...
	internal async Task OnThreadMembersUpdateEventAsync(DiscordGuild guild, ulong threadId, JArray membersAdded, JArray membersRemoved, int memberCount, CancellationToken cancellationToken = default)
	{
		...
		if (membersRemoved != null)
			...
			{
				var removedMemberId = (ulong)removedId;
				removedMemberIds.Add(removedMemberId);

				removedMembers.Add(guild.MembersInternal.TryGetValue(removedMemberId, out var member)
					? member
					: new()
					{
						Id = removedMemberId,
		...
		await this.RaiseEventAsync(this._threadMembersUpdated, threadMembersUpdateArg).ConfigureAwait(false);
	}
	...
	/*{
            try
            {
                ...
                await this.RaiseEventAsync(this._embeddedActivityUpdated, ea).ConfigureAwait(false);
            } catch (Exception ex)
            {
	...
	internal async Task OnPresenceUpdateEventAsync(JObject rawPresence, JObject rawUser)
	{
		...
		else
		{
			...
			this.PreparePresence(presence, guild);
		}
		...
		presence.RawActivities = rawPresence["activities"]?.ToObject<List<TransportActivity>?>();
		...
		else if (presence.RawActivities is not null)
		{
			if (presence.InternalActivities?.Count != presence.RawActivities.Count)
			{
				...
				for (var i = 0; i < presence.RawActivities.Count; i++)
					presence.InternalActivities.Add(new(presence.RawActivities[i]));
			}
			else
			{
		...
		await this.RaiseEventAsync(this._presenceUpdated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnUserSettingsUpdateEventAsync(TransportUser user)
	{
		...
		await this.RaiseEventAsync(this._userSettingsUpdated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnUserUpdateEventAsync(TransportUser user)
	{
		...
		await this.RaiseEventAsync(this._userUpdated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnVoiceStateUpdateEventAsync(JObject raw)
	{
		...
		var gld = this.InternalGetCachedGuild(gid);
		if (gld == null)
		{
			gld = new()
			{
			...
			_ = this.GuildsInternal.TryAdd(gid, gld);
		}
	...
	internal async Task OnVoiceChannelEffectSendEventAsync(ulong gid, ulong cid, ulong uid, JObject rawObject)
	{
		...
		await this.RaiseEventAsync(this._voiceChannelEffectSend, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnApplicationCommandCreateAsync(DiscordApplicationCommand cmd, ulong? guildId)
	{
		...
		await this.RaiseEventAsync(this._applicationCommandCreated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnApplicationCommandUpdateAsync(DiscordApplicationCommand cmd, ulong? guildId)
	{
		...
		await this.RaiseEventAsync(this._applicationCommandUpdated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnApplicationCommandDeleteAsync(DiscordApplicationCommand cmd, ulong? guildId)
	{
		...
		await this.RaiseEventAsync(this._applicationCommandDeleted, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnGuildApplicationCommandCountsUpdateAsync(int chatInputCommandCount, int userContextMenuCommandCount, int messageContextMenuCount, ulong guildId)
	{
		...
		await this.RaiseEventAsync(this._guildApplicationCommandCountUpdated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnApplicationCommandPermissionsUpdateAsync(IEnumerable<DiscordApplicationCommandPermission> perms, ulong channelId, ulong guildId, ulong applicationId)
	{
		...
		await this.RaiseEventAsync(this._applicationCommandPermissionsUpdated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnEntitlementCreateAsync(DiscordEntitlement entitlement)
	{
		...
		await this.RaiseEventAsync(this._entitlementCreated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnEntitlementUpdateAsync(DiscordEntitlement entitlement)
	{
		...
		await this.RaiseEventAsync(this._entitlementUpdated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnEntitlementDeleteAsync(DiscordEntitlement entitlement)
	{
		...
		await this.RaiseEventAsync(this._entitlementDeleted, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnSubscriptionCreateAsync(DiscordSubscription subscription)
	{
		...
		await this.RaiseEventAsync(this._subscriptionCreated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnSubscriptionUpdateAsync(DiscordSubscription subscription)
	{
		...
		await this.RaiseEventAsync(this._subscriptionUpdated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnWebhooksUpdateAsync(ulong channelId, ulong guildId)
	{
		...
		var channel = guild.GetChannel(channelId) ?? this.InternalGetCachedThread(channelId) ?? new DiscordChannel
		{
			Id = channelId,
			GuildId = guildId,
			Discord = this,
		...
		await this.RaiseEventAsync(this._webhooksUpdated, ea).ConfigureAwait(false);
	}
	...
	internal async Task OnUnknownEventAsync(GatewayPayload payload)
	{
		...
		await this.RaiseEventAsync(this._unknownEvent, ea).ConfigureAwait(false);
	}
