...
public class AutomodRule : SnowflakeObject
{
	...
	public async Task<AutomodRule> ModifyAsync(Action<AutomodRuleEditModel> action)
	{
		...
		if (mdl.TriggerMetadata.HasValue)
		{
			...
			if (mdl.TriggerMetadata.Value.AllowList != null && this.TriggerType != AutomodTriggerType.KeywordPreset)
			...
			if (mdl.TriggerMetadata.Value.MentionTotalLimit != null && this.TriggerType != AutomodTriggerType.MentionSpam)
