...
public sealed class VoiceNextConnection : IDisposable
{
		...
		while (!token.IsCancellationRequested)
		{
			...
			if (!rawPacket.Silence && this._queueCount == 0)
			{
				...
				for (var i = 0; i < 3; i++)
				{
					...
					await this.EnqueuePacketAsync(new(nullpacketmem, 20, true)).ConfigureAwait(false);
				}
			}
	...
	private bool ProcessPacket(
		ReadOnlySpan<byte> data, ref Memory<byte> opus, ref Memory<byte> pcm,
