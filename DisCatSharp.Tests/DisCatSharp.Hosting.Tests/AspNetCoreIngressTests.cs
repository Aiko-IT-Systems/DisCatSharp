...
public sealed class AspNetCoreIngressTests
{
	...
	[Fact]
	public async Task OAuthCallbackHandler_ReturnsExchangeFailureWhenDiscordExchangeThrowsHttpRequestException()
	{
		var exchangeService = new FakeOAuthTokenExchangeService(exceptionToThrow: new HttpRequestException("boom"));
		using var provider = BuildProvider(
			TimeProvider.System,
			configureOAuth: options =>
			{
	...
	[Fact]
	public async Task OAuthCallbackHandler_PropagatesUnexpectedExchangeExceptions()
	{
		...
		await store.StoreAsync(new DiscordIngressPendingState
		{
			Key = "unexpected-exchange-failure",
			RequestUri = new Uri("https://discord.com/oauth2/authorize?state=unexpected-exchange-failure&redirect_uri=https%3A%2F%2Fexample.com%2Foauth%2Fcallback")
		});

		var handler = provider.GetRequiredService<IDiscordOAuthCallbackHandler>();

		await Assert.ThrowsAsync<InvalidOperationException>(() => handler.HandleAsync(new DiscordOAuthCallbackRequest(
			code: "auth-code",
			state: "unexpected-exchange-failure",
			queryParameters: new Dictionary<string, string?>
			{
				["code"] = "auth-code",
				["state"] = "unexpected-exchange-failure"
