...
interface Statement { sql: string; params?: Scalar[] }
interface TableDelta { table: string; upserts: Statement[]; deletes: Statement[]; changed: number; deleted: number; unchanged: number }
...
const upserts = deltas.flatMap((delta) => delta.upserts);
const deletes = deltas.flatMap((delta) => delta.deletes);
const completedState: Statement = {
  sql: `UPDATE sync_state SET schema_version = ?, source_commit = ?, generated_at = ?, completed_at = ?, ready = 1,
        symbol_count = ?, document_count = ?, source_chunk_count = ?, modules_json = ?, types_json = ? WHERE id = 1`,
  params: [toD1Scalar(artifact.schemaVersion), artifact.sourceCommit, artifact.generatedAt, new Date().toISOString(), toD1Scalar(artifact.symbols.length),
...
if (fitsSingleBatch([...upserts, ...deletes, completedState])) {
  await executeBatch([...upserts, ...deletes, completedState]);
} else {
  await executeBatches(upserts);
  await executeBatches(deletes);
  await executeBatch([completedState]);
}

for (const delta of deltas) {
  console.log(JSON.stringify({ table: delta.table, changed: delta.changed, deleted: delta.deleted, unchanged: delta.unchanged }));
}

console.log(JSON.stringify({ database: databaseName, artifact: artifactPath, symbols: artifact.symbols.length, documents: artifact.documents.length, sourceChunks: artifact.sourceChunks.length, rowsRead, rowsWritten }));
...
async function createTableDelta<T extends { id: string; contentHash: string }>(table: string, records: T[], createStatement: (record: T) => Statement): Promise<TableDelta> {
  ...
  const upserts = delta.changedRecords.map(createStatement);
  const deletes: Statement[] = [];
  for (let offset = 0; offset < delta.staleIds.length; offset += 100) {
    deletes.push({ sql: `DELETE FROM ${table} WHERE record_id IN (SELECT value FROM json_each(?))`, params: [JSON.stringify(delta.staleIds.slice(offset, offset + 100))] });
  }
  return { table, upserts, deletes, changed: upserts.length, deleted: delta.staleIds.length, unchanged: delta.unchanged };
}
...
function fitsSingleBatch(statements: Statement[]): boolean {
  if (statements.length > 20) return false;
  return statements.reduce((bytes, statement) => bytes + Buffer.byteLength(JSON.stringify(statement)), 0) <= 4 * 1024 * 1024;
}
