...
class FakeDatabase {
  ...
  public constructor(
    ...
    private readonly sourceRows: Array<{ start_line: number; end_line: number; content: string }> = [],
    private readonly state: SyncStateRow = {
      ready: 1,
  ...
  public rowsFor(sql: string): unknown[] {
    if (sql.includes("FROM sync_state")) return [this.state];
...
function row(id: string, family: "symbol" | "conceptual", type: string, title: string, score: number): SearchRow {
  return { id, family, type, title, summary: "summary", url: `/${id}.html`, module: "DisCatSharp", score };
}
...
describe("SearchService", () => {
  ...
  it("fetches stable document IDs and rejects classification-prefixed legacy IDs", async () => {
    const document = { id: "document:guide", documentKey: "guide", family: "conceptual", type: "changelog", title: "Guide" };
