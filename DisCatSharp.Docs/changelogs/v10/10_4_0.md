...
- Deprecated `UseCanary` & `UsePtb`.
- Added [ApiChannel](xref:DisCatSharp.DiscordConfiguration#DisCatSharp_DiscordConfiguration_ApiChannel).

`UseCanary` & `UsePtb` temporary set `ApiChannel` internally:
[!code-csharp[](../../../DisCatSharp/DiscordConfiguration.cs#L212-L221)]
...
#### Lalas take on the username change

> [!NOTE]
 > I won't go into my opinion from the user perspective.

From a bots perspective the username system change makes much sense.
Bots having `Username#Discrim` were useless in the past because the only thing you can do with `Username#Discrim` is to add friends, which you can't do with bots.
A single `@username` makes much more sense for bots.
