...
- Fixed a bug various flag enums where the `[Flags]` annotation was missing.
...
- Removed `GuildBans` from [DiscordIntents](xref:DisCatSharp.Enums.DiscordIntents) and replaced it with `GuildModeration`.
...
Additionally we've added the function [TryGetPublishedListingsAsync](xref:DisCatSharp.DiscordClient#DisCatSharp_DiscordClient_TryGetPublishedListingsAsync_System_UInt64_) to get the published premium sku listings for a application.
