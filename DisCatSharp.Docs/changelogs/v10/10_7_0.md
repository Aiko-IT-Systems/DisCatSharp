...
`DisCatSharp.Voice` has shipped as the new voice implementation, replacing `DisCatSharp.VoiceNext`. It includes full DAVE end-to-end encryption support using Discord's MLS-based ratchet protocol (`libdave`), improved memory safety, and restored audio receive support.
...
- **`DisCatSharp.Voice`** — New Discord voice implementation replacing `DisCatSharp.VoiceNext`. Includes full support for DAVE end-to-end encryption (AES-128-GCM per-frame via MLS ratchets), audio send and receive, and a `libdave`-based MLS backend. All VoiceNext APIs are available under the `DisCatSharp.Voice` namespace with renamed types (`VoiceConnection`, `VoiceConfiguration`, `VoiceExtension`).
