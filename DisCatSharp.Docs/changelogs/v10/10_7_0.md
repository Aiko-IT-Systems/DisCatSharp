...
- **Application Commands**: Removed `dmPermission` fields, causing DisCatSharp to do weird bulk-updates. Use `allowed_contexts` instead.
...
- [`DiscordClient.ApiChannel`](xref:DisCatSharp.DiscordClient.ApiChannel)
- [`DiscordClient.ApiVersion`](xref:DisCatSharp.DiscordClient.ApiVersion)
- [`DiscordClient.Statistics`](xref:DisCatSharp.DiscordClient.Statistics)
...
- [`DiscordConfiguration? DiscordOAuth2Client.DiscordConfiguration`](xref:DisCatSharp.DiscordOAuth2Client.DiscordConfiguration)
...
- [`DiscordGuild.TryGetMember`](xref:DisCatSharp.Entities.DiscordGuild.TryGetMember)
...
- [`DisCatSharpBuilder.CheckComponentIds`](xref:DisCatSharp.DisCatSharpBuilder.CheckComponentIds)
- [`DisCatSharpBuilder.AddId`](xref:DisCatSharp.DisCatSharpBuilder.AddId)
...
- Fixed naming on `DiscordClient`: `CreateTestEntitlementsAsync` renamed to `CreateTestEntitlementAsync` and `DeleteTestEntitlementsAsync` renamed to `DeleteTestEntitlementAsync`.
- Fixed the `Recalculate` function for interactivity pages.
- Fixed issues with the `DiscordOAuth2Client`.
- Fixed issues with the `OAuthWeb` extension when adding users to a guild.
